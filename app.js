...
const LEX = [
  ...
  { id: 'broker', n: T('Broker (Courtier)', 'Broker'), alt: T('Intermédiaire de marché', 'Market intermediary'), c: 'basics', lvl: 1, rel: ['spread', 'leverage', 'margin', 'regulator'],
    d: T("Plateforme ou société par laquelle tu passes pour acheter et vendre sur les marchés. Elle exécute tes ordres, peut proposer du levier, et prélève des coûts (spread, parfois commissions).", "A platform or company through which you buy and sell in the markets. It executes your orders, may offer leverage, and charges costs (spread, sometimes commissions)."),
    ex: T("Un trader ouvre un compte chez un broker pour accéder au Forex et aux actions.", "A trader opens an account with a broker to access Forex and stocks.") },
  { id: 'regulator', n: T('Régulateur financier', 'Financial regulator'), alt: T('Autorité de contrôle', 'Supervisory authority'), c: 'basics', lvl: 1, rel: ['broker'],
    d: T("Autorité publique qui encadre les brokers et les marchés financiers d'un pays ou d'une région, pour protéger les investisseurs (par exemple l'AMF en France, la FCA au Royaume-Uni). Vérifier qu'un broker est régulé est une précaution de base.", "A public authority that oversees brokers and financial markets in a country or region, to protect investors (for example the AMF in France, the FCA in the UK). Checking that a broker is regulated is a basic precaution."),
    ex: T("Avant d'ouvrir un compte, on peut vérifier le numéro d'agrément d'un broker sur le site de son régulateur.", "Before opening an account, you can check a broker's licence number on its regulator's website.") },
  { id: 'demo-account', n: T('Compte démo', 'Demo account'), alt: T('Compte de simulation', 'Practice account'), c: 'basics', lvl: 1, rel: ['broker'],
    d: T("Compte proposé par un broker pour s'entraîner avec de l'argent fictif, dans des conditions proches du marché réel. Utile pour apprendre l'interface et tester une méthode sans risquer de capital, même si l'exécution peut différer légèrement d'un compte réel.", "An account offered by a broker to practise with fictional money, under near-real market conditions. Useful for learning the interface and testing a method without risking capital, though execution can differ slightly from a real account."),
    ex: T("Avant de trader en argent réel, beaucoup de débutants s'entraînent plusieurs semaines sur un compte démo.", "Before trading real money, many beginners practise for several weeks on a demo account.") },
  { id: 'commission', n: T('Commission', 'Commission'), alt: T('Frais par transaction', 'Fee per trade'), c: 'basics', lvl: 1, rel: ['spread', 'broker'],
    d: T("Frais fixe ou proportionnel prélevé par certains brokers à l'ouverture ou à la clôture d'une position, en plus ou à la place du spread. Il s'ajoute au coût réel d'un trade.", "A fixed or proportional fee charged by some brokers when opening or closing a position, in addition to or instead of the spread. It adds to the real cost of a trade."),
    ex: T("Un broker peut facturer une commission par lot échangé, en plus d'un spread réduit.", "A broker may charge a commission per lot traded, in addition to a reduced spread.") },
  { id: 'cfd', n: T('CFD (Contrat sur la différence)', 'CFD (Contract for Difference)'), alt: T('Produit dérivé sans détention', 'Derivative without ownership'), c: 'basics', lvl: 2, rel: ['leverage', 'margin'],
    d: T("Produit dérivé qui permet de spéculer sur la variation d'un prix sans posséder l'actif sous-jacent. Il utilise souvent du levier, ce qui amplifie gains et pertes, et sa réglementation varie selon les pays.", "A derivative product that lets you speculate on a price's movement without owning the underlying asset. It often uses leverage, which amplifies gains and losses, and its regulation varies by country."),
    ex: T("Un CFD sur une action permet de profiter de sa hausse ou de sa baisse sans détenir réellement l'action.", "A CFD on a stock lets you profit from its rise or fall without actually owning the stock.") },
  { id: 'swap', n: T('Swap (frais de rollover)', 'Swap (rollover fee)'), alt: T('Coût de financement au jour le jour', 'Overnight financing cost'), c: 'basics', lvl: 2, rel: ['leverage', 'broker'],
    d: T("Frais ou gain quotidien appliqué quand une position reste ouverte au-delà d'un jour de trading, lié à la différence de taux d'intérêt entre devises ou au financement de la position. Il peut être positif ou négatif selon le sens du trade.", "A daily fee or gain applied when a position stays open beyond one trading day, linked to the interest-rate difference between currencies or to the cost of financing the position. It can be positive or negative depending on the trade direction."),
    ex: T("Garder une position ouverte plusieurs nuits sur le Forex entraîne généralement des frais ou gains de swap chaque jour.", "Keeping a position open for several nights on Forex usually results in a daily swap fee or gain.") },
  ...
  { id: 'oco-order', n: T('Ordre OCO', 'OCO order'), alt: T("L'un annule l'autre", 'One Cancels the Other'), c: 'orders', lvl: 3, rel: ['stop-order', 'limit-order'],
    d: T("Paire de deux ordres liés (par exemple un stop et une limite) où l'exécution de l'un annule automatiquement l'autre. Utile pour couvrir deux scénarios opposés sans surveiller le marché en continu.", "A pair of two linked orders (for example a stop and a limit) where the execution of one automatically cancels the other. Useful for covering two opposite scenarios without watching the market continuously."),
    ex: T("Un ordre OCO peut combiner une entrée si le prix casse une résistance et une autre si le prix casse un support, en sens inverse.", "An OCO order can combine an entry if price breaks a resistance and another if price breaks a support, in the opposite direction.") },
  { id: 'trailing-stop', n: T('Stop suiveur (Trailing stop)', 'Trailing stop'), alt: T('Stop qui suit le prix', 'Stop that follows price'), c: 'orders', lvl: 2, rel: ['stop-loss', 'take-profit'],
    d: T("Stop-loss qui se déplace automatiquement pour suivre le prix quand il évolue favorablement, tout en restant fixe s'il repart contre la position. Il permet de protéger une partie des gains sans fixer un objectif figé.", "A stop-loss that automatically moves to follow price as it moves favourably, while staying fixed if price turns against the position. It helps protect part of the gains without setting a fixed target."),
    ex: T("Un stop suiveur de 20 points remonte avec le prix en cas de hausse, mais ne redescend jamais.", "A 20-point trailing stop rises with price on the way up, but never moves back down.") },
  { id: 'hedging', n: T('Couverture (Hedging)', 'Hedging'), alt: T('Position compensatoire', 'Offsetting position'), c: 'orders', lvl: 3, rel: ['leverage', 'market-structure'],
    d: T("Pratique consistant à ouvrir une position pour compenser le risque d'une autre, par exemple une position opposée sur un actif corrélé. Elle réduit l'exposition mais aussi le gain potentiel, et certains brokers ou réglementations l'encadrent différemment.", "The practice of opening a position to offset the risk of another, for example an opposite position on a correlated asset. It reduces exposure but also potential gain, and some brokers or regulations treat it differently."),
    ex: T("Un trader qui possède des actions d'une entreprise peut se couvrir en vendant à découvert un indice du même secteur.", "A trader holding a company's shares can hedge by short-selling an index from the same sector.") },
  ...
  { id: 'central-bank', n: T('Banque centrale', 'Central bank'), alt: T('Institution monétaire', 'Monetary institution'), c: 'analysis', lvl: 2, rel: ['interest-rate', 'economic-calendar'],
    d: T("Institution qui fixe la politique monétaire d'une zone (taux d'intérêt, quantité de monnaie en circulation). Ses décisions et discours influencent fortement les devises et, plus largement, les marchés.", "An institution that sets the monetary policy of a region (interest rates, money supply). Its decisions and statements strongly influence currencies and markets more broadly."),
    ex: T("La Réserve fédérale américaine (Fed) et la Banque centrale européenne (BCE) sont deux banques centrales suivies de près par les traders Forex.", "The US Federal Reserve (Fed) and the European Central Bank (ECB) are two central banks closely followed by Forex traders.") },
  { id: 'interest-rate', n: T('Taux directeur', 'Interest rate'), alt: T('Taux de la banque centrale', "Central bank's rate"), c: 'analysis', lvl: 2, rel: ['central-bank', 'inflation'],
    d: T("Taux fixé par une banque centrale, qui influence le coût du crédit et l'attractivité d'une devise. Une hausse attire souvent les capitaux vers cette devise ; une baisse peut la rendre moins attractive.", "A rate set by a central bank, which influences the cost of credit and how attractive a currency is. A rise often draws capital toward that currency; a cut can make it less attractive."),
    ex: T("Quand une banque centrale relève son taux directeur, la devise du pays réagit souvent le jour même.", "When a central bank raises its interest rate, the country's currency often reacts the same day.") },
  { id: 'inflation', n: T('Inflation', 'Inflation'), alt: T('Hausse générale des prix', 'General rise in prices'), c: 'analysis', lvl: 1, rel: ['interest-rate', 'central-bank'],
    d: T("Hausse générale et durable des prix, qui réduit le pouvoir d'achat d'une monnaie. Les banques centrales ajustent souvent leur taux directeur pour la maîtriser, ce qui a des répercussions sur les marchés.", "A general and lasting rise in prices, which reduces a currency's purchasing power. Central banks often adjust their interest rate to control it, which has knock-on effects on markets."),
    ex: T("Une inflation plus forte que prévu peut pousser une banque centrale à relever ses taux.", "Higher-than-expected inflation can push a central bank to raise its rates.") },
  { id: 'economic-calendar', n: T('Calendrier économique', 'Economic calendar'), alt: T('Agenda des annonces', 'Schedule of announcements'), c: 'analysis', lvl: 1, rel: ['volatility', 'slippage'],
    d: T("Liste des annonces économiques à venir (taux, emploi, inflation, résultats d'entreprises…) avec leur date, heure et niveau d'importance. Il aide à anticiper les moments de volatilité plus élevée.", "A list of upcoming economic announcements (rates, employment, inflation, company earnings…) with their date, time and importance level. It helps anticipate moments of higher volatility."),
    ex: T("Avant de trader, on peut consulter le calendrier économique pour repérer les annonces importantes de la journée.", "Before trading, you can check the economic calendar to spot the day's important announcements.") },
  ...
  { id: 'fibonacci-retracement', n: T('Retracement de Fibonacci', 'Fibonacci retracement'), alt: T('Niveaux de retour du prix', 'Price pullback levels'), c: 'analysis', lvl: 3, rel: ['support', 'resistance'],
    d: T("Outil qui trace des niveaux horizontaux (souvent 38,2 %, 50 %, 61,8 %) entre un plus haut et un plus bas, pour repérer des zones où un retour du prix pourrait s'arrêter avant de reprendre sa direction. Ce sont des repères statistiques, pas des règles garanties.", "A tool that draws horizontal levels (often 38.2%, 50%, 61.8%) between a high and a low, to spot zones where a price pullback might stop before resuming direction. These are statistical reference points, not guaranteed rules."),
    ex: T("Après une forte hausse, un trader peut surveiller le niveau de retracement à 61,8 % comme zone de rebond possible.", "After a strong rise, a trader might watch the 61.8% retracement level as a possible bounce zone.") },
  { id: 'fomo', n: T('FOMO (peur de manquer une opportunité)', 'FOMO (Fear Of Missing Out)'), alt: T('Impulsion à entrer trop tard', 'Impulse to enter too late'), c: 'analysis', lvl: 1, rel: ['overtrading', 'journal'],
    d: T("Impulsion à entrer précipitamment sur un marché après un mouvement déjà bien engagé, par peur de « rater » un gain. Elle pousse souvent à entrer tard, sans plan ni invalidation claire.", "The impulse to rush into a market after a move is already well underway, out of fear of “missing” a gain. It often leads to entering late, with no plan or clear invalidation."),
    ex: T("Voir un prix monter fortement peut donner envie d'acheter dans l'instant, même sans setup respecté : c'est un exemple typique de FOMO.", "Seeing a price rise sharply can create the urge to buy immediately, even without a valid setup: a typical example of FOMO.") },
  { id: 'overtrading', n: T('Surtrading (Overtrading)', 'Overtrading'), alt: T('Trop de trades', 'Too many trades'), c: 'analysis', lvl: 2, rel: ['fomo', 'journal'],
    d: T("Fait de prendre trop de trades, souvent après une perte ou pour « se refaire », au-delà de ce que prévoit sa méthode. Il augmente les coûts (spread, commissions) et le risque global.", "Taking too many trades, often after a loss or to “win it back”, beyond what one's method calls for. It raises costs (spread, commissions) and overall risk."),
    ex: T("Après trois pertes d'affilée, multiplier les trades pour compenser rapidement est une forme fréquente de surtrading.", "After three losses in a row, taking many more trades to quickly recover is a common form of overtrading.") },
  { id: 'confirmation-bias', n: T('Biais de confirmation', 'Confirmation bias'), alt: T('Ne voir que ce qui confirme', 'Only seeing what confirms'), c: 'analysis', lvl: 2, rel: ['confluence', 'journal'],
    d: T("Tendance à privilégier les informations qui confirment une idée déjà en tête, et à minimiser celles qui la contredisent. En trading, il peut pousser à ne voir que les signaux qui vont dans le sens souhaité.", "The tendency to favour information that confirms an idea already in mind, while playing down what contradicts it. In trading, it can lead to seeing only the signals that point the way one wants."),
    ex: T("Chercher des confluences jusqu'à en trouver, en ignorant les signes contraires, est une manifestation du biais de confirmation.", "Searching for confluences until finding some, while ignoring contrary signs, is a manifestation of confirmation bias.") },
  ...
  { id: 'ath-atl', n: T('ATH / ATL (records historiques)', 'ATH / ATL (all-time high/low)'), alt: T('Plus haut et plus bas jamais atteints', 'Highest and lowest ever reached'), c: 'structure', lvl: 1, rel: ['support', 'resistance'],
    d: T("ATH (All-Time High) désigne le plus haut niveau jamais atteint par un prix ; ATL (All-Time Low), le plus bas. Ce sont des repères souvent surveillés, sans garantie qu'ils tiennent ou soient dépassés.", "ATH (All-Time High) is the highest level a price has ever reached; ATL (All-Time Low) is the lowest. These are widely watched reference points, with no guarantee they will hold or be broken."),
    ex: T("Quand une cryptomonnaie dépasse son ATH précédent, elle atteint un niveau de prix jamais vu auparavant.", "When a cryptocurrency exceeds its previous ATH, it reaches a price level never seen before.") },
  ...
  { id: 'channel', n: T('Canal (de tendance)', 'Channel'), alt: T('Deux lignes parallèles', 'Two parallel lines'), c: 'structure', lvl: 2, rel: ['trend', 'support'],
    d: T("Deux lignes parallèles qui encadrent les hauts et les bas du prix en tendance, l'une servant de support dynamique et l'autre de résistance dynamique. Une sortie du canal peut signaler une accélération ou une fin de tendance.", "Two parallel lines framing the highs and lows of a trending price, one acting as dynamic support and the other as dynamic resistance. Exiting the channel can signal an acceleration or the end of the trend."),
    ex: T("Dans un canal haussier, le prix rebondit régulièrement sur la ligne basse et se rapproche de la ligne haute.", "In a rising channel, price regularly bounces off the lower line and moves toward the upper line.") },
  { id: 'gap', n: T('Gap (écart de prix)', 'Gap'), alt: T('Écart entre deux bougies', 'Gap between two candles'), c: 'structure', lvl: 2, rel: ['volatility', 'support'],
    d: T("Écart entre la clôture d'une période et l'ouverture de la suivante, sans échange de prix entre les deux, souvent visible après une fermeture de marché ou une actualité importante. Il peut être comblé rapidement ou rester un niveau durable.", "A gap between one period's close and the next period's open, with no trading in between, often seen after a market closure or major news. It can be filled quickly or remain a lasting level."),
    ex: T("Une action peut ouvrir 5 % plus haut que sa clôture de la veille après une bonne nouvelle publiée hors séance : c'est un gap haussier.", "A stock can open 5% above its previous close after good news released outside trading hours: that's a bullish gap.") },
